...
async updateProfilePicture(jid, img) {
   ...
   return this.sendMessage(jid, await convert(medias.data, null, opts.packname || 'ANDY-BOTZ', opts.author || this.user.name), 'stickerMessage', { quoted: m, ...opts })
   }
...
`.trim()
    ...
    */
    ...
    async sendButtonImg(jid, content, url, footer, button1, row1, options = {}) {
      const media = await this.prepareMessage(jid, {url:`${url}`}, MessageType.image, { thumbnail: Buffer.alloc(0) })// change for file type
